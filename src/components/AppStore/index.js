...
class AppStore extends Component {
  state = {activeKeyword: '', activeTabId: tabsList[0].tabId}

  searchKeyword = event => {
    const searchedKey = event.target.value
    this.setState({activeKeyword: searchedKey})
  }
  ...
  render() {
    const {activeKeyword, activeTabId} = this.state
    const filteredList = appsList.filter(
      eachItem => activeTabId === eachItem.category,
    )
    const searchedItems = filteredList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(activeKeyword.toLowerCase()),
    )
    return (
      <div className="app-container">
        ...
        <ul className="appsContainer">
          {searchedItems.map(eachApp => (
            <AppItem key={eachApp.appId} appsEach={eachApp} />
          ))}
        </ul>
      </div>
    )
